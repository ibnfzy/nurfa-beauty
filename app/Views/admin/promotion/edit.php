...
<div x-data="{
    ...
    isActive: <?= ($promotion['is_active'] ?? 0) ? 'true' : 'false' ?>,
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-xl font-semibold text-gray-800">Edit Promosi</h2>
            <p class="text-sm text-gray-500 mt-1">Perbarui informasi promosi</p>
        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                required
                                class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20">
                                <option value="discount" <?= old('type', $promotion['type'] ?? '') === 'discount' ? 'selected' : '' ?>>Diskon</option>
                                <option value="voucher" <?= old('type', $promotion['type'] ?? '') === 'voucher' ? 'selected' : '' ?>>Voucher</option>
                                <option value="flash_sale" <?= old('type', $promotion['type'] ?? '') === 'flash_sale' ? 'selected' : '' ?>>Flash Sale</option>
                                <option value="birthday" <?= old('type', $promotion['type'] ?? '') === 'birthday' ? 'selected' : '' ?>>Birthday</option>
                            </select>
                        </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        id="voucher_count"
                                        name="voucher_count"
                                        value="<?= old('voucher_count', $promotion['voucher_count'] ?? '') ?>"
                                        min="1"
                                        max="100"
                                        class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20"
                                        placeholder="1">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        value="<?= old('voucher_validity_days', $promotion['voucher_validity_days'] ?? '') ?>"
                                        min="1"
                                        class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20"
                                        placeholder="30">
                                </div>
                ...
                <?php if (!empty($vouchers)): ?>
                    <div class="bg-white rounded-xl shadow-sm border border-primary-light/30 overflow-hidden">
                        <div class="p-6 border-b border-gray-100">
                            <h3 class="font-semibold text-gray-800">Voucher yang Dihasilkan</h3>
                            <p class="text-sm text-gray-500 mt-1">Daftar voucher yang sudah dibuat untuk promosi ini</p>
                        </div>
                        <?php
                        $voucherStatusColors = [
                            ...
                            <table class="w-full">
                                <thead>
                                    <tr class="bg-cream/50">
                                        <th class="text-left py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Kode</th>
                                        <th class="text-left py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Pelanggan</th>
                                        <th class="text-left py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                                        <th class="text-left py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Digunakan Pada</th>
                                        <th class="text-left py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Kadaluarsa</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100">
                                    <?php foreach ($vouchers as $voucher): ?>
                                        <tr class="hover:bg-cream/50 transition-colors">
                                            <td class="py-3 px-4">
                                                <code class="px-2 py-1 bg-gray-100 rounded text-sm font-mono font-medium text-gray-700"><?= esc($voucher['code'] ?? '') ?></code>
                                            </td>
                                            <td class="py-3 px-4 text-gray-600">
                                                <?php if (!empty($voucher['customer_name'])): ?>
                                                    <?= esc($voucher['customer_name']) ?>
                                                <?php else: ?>
                                            ...
                                            <td class="py-3 px-4">
                                                <?php
                                                $vs = $voucher['status'] ?? 'unused';
                                                $vsc = $voucherStatusColors[$vs] ?? $voucherStatusColors['unused'];
                                                $vsl = $voucherStatusLabels[$vs] ?? 'Tidak Diketahui';
                                                ?>
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?= $vsc ?>"><?= $vsl ?></span>
                                            </td>
                                            <td class="py-3 px-4 text-gray-600 text-sm">
                                                <?= ($voucher['used_at'] ?? '') ? date('d M Y H:i', strtotime($voucher['used_at'])) : '-' ?>
                                            </td>
                                            <td class="py-3 px-4 text-gray-600 text-sm">
                                                <?= ($voucher['expires_at'] ?? '') ? date('d M Y', strtotime($voucher['expires_at'])) : '-' ?>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-primary-light/30 p-6">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            name="target_segment"
                            class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20">
                            <option value="all" <?= old('target_segment', $promotion['target_segment'] ?? 'all') === 'all' ? 'selected' : '' ?>>Semua Pelanggan</option>
                            <option value="new" <?= old('target_segment', $promotion['target_segment'] ?? '') === 'new' ? 'selected' : '' ?>>Pelanggan Baru</option>
                            <option value="loyal" <?= old('target_segment', $promotion['target_segment'] ?? '') === 'loyal' ? 'selected' : '' ?>>Pelanggan Loyal</option>
                            <option value="vip" <?= old('target_segment', $promotion['target_segment'] ?? '') === 'vip' ? 'selected' : '' ?>>VIP</option>
                        </select>
                    </div>
                </div>
